<template>
  <div class="wrapper" :model="form">
    <!-- header部分 -->
    <header>
      <div class="location-text"><i class="iconfont icon-angle-left" @click="toback"></i>编辑送货地址</div>
    </header>
    <!-- 表单部分 -->
    <ul class="form-box">
      <li>
...
姓名">
      ...
      <li>
        ...
        <div class="content">
          <input type="text" v-model="form.deliveryAddress.address" placeholder="收货地址">
        </div>
      </li>
    </ul>
...
const getDeliveryAddressById = async () => {
  ...
  const res=await deliveryAddressStore.getDeliveryAddressById(form.value.daId)
  form.value.deliveryAddress=deliveryAddressStore.deliveryAddressList;
}
...
const editUserAddress = async () => {
  ...
  const res= await deliveryAddressStore.updateDeliveryAddress(form.value.deliveryAddress)
  if(res.code==globalProperties.$code_success) {
    ElMessage({type:'success',message:"更新成功"})
    router.push({path: '/userAddress',query: {shopId:form.value.shopId}});
  }else {
    ElMessage({type:'warning',message:"更新地址失败"})
  }}
